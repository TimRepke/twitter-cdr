...
WITH tmp AS (SELECT ti.item_id,
                    ...
                    ti.twitter_author_id,
                    (ti."user" -> 'created_at')::text::timestamp                                        as created,
                    extract('day' from date_trunc('day', '2022-12-31 23:59'::timestamp - (ti."user" -> 'created_at')::text::timestamp)) as days,
                    (ti."user" -> 'tweet_count')::int                                                   as n_tweets
             FROM twitter_item ti
                      LEFT JOIN bot_annotation ba_tech ON (
                         ti.item_id = ba_tech.item_id
     ...
     users_paneled AS (SELECT users.*,
                              CASE
                                  ...
                                  END as panel
                       FROM users)
SELECT panel, twitter_author_id, "Positive","Neutral", "Negative", n_cdr_tweets, n_tweets, days, n_tweets / days as tpd
FROM users_paneled;