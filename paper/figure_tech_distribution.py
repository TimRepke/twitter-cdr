...
with open('data/user_stats.pkl', 'rb') as f:
    data: list[dict] = [r.model_dump() for r in pickle.load(f)]
...
TECHNOLOGIES = [
    # 'Methane Removal',  # 0
    # 'CCS',  # 1
    ('c', 'Afforestation/Reforestation'),  # 6
    ('c', 'Ecosystem Restoration'),  # 7
    ('c', 'Soil Carbon Sequestration'),  # 8
    ('c', 'Blue Carbon'),  # 10
    ('n', 'Biochar'),  # 5
    ('n', 'BECCS'),  # 9
    ('n', 'Ocean Fertilization'),  # 2
    ('n', 'Ocean Alkalinization'),  # 3
    ('n', 'Enhanced Weathering'),  # 4
    ('n', 'Direct Air Capture'),  # 11
    ('o', 'GGR (general)'),  # 12
]
...
def make_figure(mask, fname):
    ...
    checksum = 0
    entries = {'c': [], 'n': [], 'o': []}
    for ti, (nc, technology) in enumerate(TECHNOLOGIES):
        normed = (tech_counts[technology] / stack_totals) * 100
        entries[nc].append(ax.bar(buckets, normed, width, label=technology, bottom=bottom, color=cmap(ti)))
    ...
    for (_, technology) in reversed(TECHNOLOGIES):
    ...
    legend_1 = fig.legend(handles=entries['o'], reverse=True, loc='outside lower right', bbox_to_anchor=(0, .13, 1, 1),
                          fontsize=18)
    fig.add_artist(legend_1)
    legend_2 = fig.legend(handles=entries['n'], ncols=2, title='Novel CDR', reverse=True, loc='outside lower left',
                          fontsize=18)
    fig.add_artist(legend_2)
    fig.legend(handles=entries['c'], ncols=2, reverse=True, title='Conventional CDR', loc='outside lower right',
               fontsize=18)
